...
using UWP.Media;//using Android.Media;
//using Android.OS;
//using Java.Nio;
...
namespace xnano.UWP.Gamestream
{
    public class AudioDecoder : IDisposable
    {
        ...
        public static MediaFormat GetMediaFormat(string mimeType, int sampleRate, int channels)
        {
            ...
            format.SetByteBuffer("csd-0", /*Java.Nio.ByteBuffer.Wrap(csd0)*/NioByteBuffer.Wrap(csd0));
        ...
        private AudioTrack GetAudioTrack()
        {
            ...
            int bufferSize = default;//AudioTrack.GetMinBufferSize(_sampleRate, channelOut, encoding) * 2;
                ...
                try
                {
                    while (!_disposed)
                    {
                        ...
                        if (outIndex >= 0)
                        {
                            ...
                            /*ByteBuffer*/var outputBuffer = _mediaCodec.GetOutputBuffer(lastIndex);
            ...
            var decoderThread = new Thread(() =>
            {
                try
                {
                    while (!_disposed)
                    {
                        ...
                        {
                            /*ByteBuffer*/var inputBuffer = _mediaCodec.GetInputBuffer(inputBufferIndex);
                            if (inputBuffer != null)
                            {
                                byte[] sample = null;
                                do
